...
/**
 * Created by maodongwei on 2018/7/20.
 *  okHttp网络请求工具类
 */
...
public class RequestManager {
    ...
    private static final MediaType MEDIA_TYPE_JSON = MediaType.parse("application/json");// Content-Type: mdiatype 这个需要和服务端保持一致
    private static final MediaType MEDIA_TYPE_MARKDOWN = MediaType.parse("text/x-markdown; charset=utf-8");// Content-Type: mdiatype 这个需要和服务端保持一致
    private static final String BASE_URL = "http://bs.rockemb.net/api/v10";//请求接口根地址
     ...
     * okHttp post异步请求
     * @param actionUrl 接口地址
     * @param reqMsgJson 请求信息参数 json格式
     * @param callBack 请求返回数据回调
     */
    public void requestPostByAsyn(String actionUrl, String reqMsgJson, final ReqCallBack callBack) {
        try {
            RequestBody body = RequestBody.create(MEDIA_TYPE_JSON, reqMsgJson);
            String requestUrl = String.format("%s/%s", BASE_URL, actionUrl);
            final Request request = addHeaders().url(requestUrl).post(body).build();
            final Call call = mOkHttpClient.newCall(request);
            call.enqueue(new Callback() {
...
//                .addHeader("client", "BMACH")
//                .addHeader("token", "")
//                .addHeader("mode", "remote")
//                .addHeader("method", "POST")
//                .addHeader("url", "")
//                .addHeader("action", "remote")
//                .addHeader("formId", "POST")
//                .addHeader("errorCallback", null);
//        return builder;
//    }
    ...
    /**
     *
     * sender （ReportBookInfo接口需要的参数）根据上传资产编号接口动态获取。接口body如下：
     * {
     "head": {
        "version": "1.0",
        "client": "BMACH",
