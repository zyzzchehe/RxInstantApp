...
//				}
	...
	public Locker(Context context) {
		Log.i("zyz","Constant.COM_LOCKER=="+Constant.COM_LOCKER);

		Constant.COM_LOCKER = "/dev/ttyUSB0";
		Constant.writeLog("zyz com jiedian: "+Constant.COM_LOCKER);
		mBase = context;
		try {
...
//						mHandler.sendEmptyMessage(2);
    ...
    //开始上电解锁
	public void givePower(int time){
		int t = Integer.parseInt(Integer.toHexString(time),16);
		lockOrder = new byte[8];
		lockOrder[0] = (byte) 0xAA;
		lockOrder[1] = 0x55;
		lockOrder[2] = 0x04;
		...
		lockOrder[4] = 0x50;
		lockOrder[5] = 0x00;
		lockOrder[6] = (byte)(t);//开锁时间
		Log.d("zyz","lockOrder[6] == "+Integer.toHexString(time));
		lockOrder[7] = Constant.calcCrc8(lockOrder, 0, 7);//校验
		//lock.start();
		lock.sendData(lockOrder);//锁控板的类发送数据
	}

	public boolean openLock(String boardStr, String lockStr) throws InterruptedException {
		n = 1;
		boxId = lockStr;
		isBusy = true;
		lockOrder = new byte[7];
		...
		lockOrder[6] = Constant.calcCrc8(lockOrder, 0, 6);
		lock.start();
		return lock.sendData(lockOrder);
//		while (LISHIZHI == null) {
//			Thread.sleep(100);
//		}
//
